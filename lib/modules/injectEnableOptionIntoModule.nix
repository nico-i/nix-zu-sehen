...
let
  ...
  evaluatedModule = if builtins.pathExists modulePath then
    ...
    throw
    "injectEnableOptionIntoModule: `modulePath` must be an existing file or dir, got: '${
      toString modulePath
    }'";

  enableOptionValue = lib.mkEnableOption "enable ${moduleName} configuration";
  enableOption = if builtins.isString moduleCategoryName then {
    ${customConfigName}.${moduleCategoryName}.${moduleName}.enable =
      enableOptionValue;
  } else {
...
in {
  ...
  config = (lib.mkIf (if builtins.isString moduleCategoryName then
    customConfig.${moduleCategoryName}.${moduleName}.enable
  else
    customConfig.${moduleName}.enable) evaluatedModule.config);
}
